...
def test_generalized_perspective_coordinate():
    ...
    b_correct = np.allclose(
        generalized_perspective.b, [-0.04515077, -0.10718482], atol=1e-6
    )
    c_correct = np.allclose(
        generalized_perspective.c, [0.00352531, 0.00488361], atol=1e-6
    )
    stretch_correct = np.allclose(
        generalized_perspective.stretch_factor, [-0.00362438, -0.00223337], atol=1e-6
    )
    stretch_center_correct = np.allclose(
        generalized_perspective.stretch_center_off,
        [-7.57650567, -0.62730542],
        atol=1e-6,
    )
    bulge_correct = np.allclose(
        generalized_perspective.bulge_factor,
        [-3.66546709e-04, 6.25126767e-01],
        atol=1e-6,
    )
    bulge_center_correct = np.allclose(
    ...
    all_correct = (
        ...
        and c_correct
        and stretch_correct
        and stretch_center_correct
        and bulge_correct
        and bulge_center_correct
    )
    if all_correct:
        assert all_correct
    else:
        pytest.xfail("Generalized perspective transformation is not correct.")
