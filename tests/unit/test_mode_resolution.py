...
def test_resolve_colorchannel_rgb_r() -> None:
    arr = np.array(
        [
            [[0, 0, 0], [255, 0, 0]],
            [[127, 10, 10], [64, 0, 0]],
    ...
    signal = resolve_mode_image("colorchannel.rgb.r", img)
...
def test_simple_segmentation_supports_colorchannel_mode_without_mass_inputs() -> None:
    ...
    img = _optical_image(arr)
    segmentation = SimpleSegmentation(mode="colorchannel.rgb.r", threshold=0.5)
    mask = segmentation(
        img,
        saturation_g=None,
...
def test_mode_requires_color_to_mass_for_new_and_legacy_modes() -> None:
    ...
    assert not mode_requires_color_to_mass("colorchannel.rgb.r")
