...
class BinaryMassAnalysis:
    ...
    Attributes:
        base_image (darsia.Image): A base darsia image. In principle only needed for size properties,
            like numer of pixels, height and width.
        pixelarea (float): product of dx and dy from base_image.
        porosity (np.ndarray): porosity map of equal size to base_image. Default is array of ones.
        depth_map (np.ndarray): depth map contain information about depth associated to each pixel.
            Default is an array of ones.
        height_map (np.ndarray): Associates each pixel with its distance from the top of the image.
    """
    ...
    def __init__(
        ...
        Arguments:
            base_image (darsia.Image): In principle only needed for size properties, 
                like numer of pixels, height and width.
                depth_map (Optional[np.ndarray]): depth map with depth associated to each pixel.
                depth_measurements (Optional[tuple[np.ndarray, ...]]): tuple of depth measurements. 
                    Should be provided as horizontal coordinates and vertical coordinates as a 
                    meshgrid, and depth cooridinates corresponding to each horizontal and vertical entry.
                porosity (Optional[Union[np.ndarray, float]]): porosity map. Can be provided as a uniform 
                    scalar value as well.
        ...
        if depth_measurements is not None:
            ...
            warn(
                ...
                " the segmented images that are to be analyzed) or a tuple of depth measurements."
                " Now, a constant depth of 1 is assumed."
            )
            self.depth_map = np.ones(self.base_image.img.shape[:2])
        
        # Defines a height map associating each pixel with its physical height (distance from top)
    ...
    def volume_map(self, segmentation: np.ndarray, component: int) -> np.ndarray:
        ...
        Arguments:
            ...
            component (int): the value that the component of interest takes in the segmentation.
        ...
        if self.depth_map.shape != segmentation.shape:
            if (
                ...
                raise Exception(
                    ...
                    " Please make sure that the depthmap is at least as large as the segmentation."
                    " For optimal accuracy they should be equal in size."
                )
            warn(
                f"The shape of the depthmap is {self.depth_map.shape} and the shape of the"
                f" segmentations is {segmentation.shape}. These should be the same for optimal accuracy."
            )
    ...
    def free_co2_mass(
        ...
        Arguments:
            segmentation (np.ndarray): segmented image
            external_pressure (float): external pressure, for example atmospheric pressure.
            co2_component (int): value of the component in segmentation that corresponds to co2.
    ...
    def _compute_depth_map(self, depth_measurements: tuple[np.ndarray, ...]) -> np.ndarray:
        """
        Compute depth map, based on the reported measurements.

        Arguments:
             depth_measurements (Optional[tuple[np.ndarray, ...]]): tuple of depth measurements. 
                    Should be provided as horizontal coordinates, vertical coordinates, and depth
                    cooridinates corresponding to each horizontal and vertical entry.
        Returns:
