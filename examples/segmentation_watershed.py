...
markers_basis_hist = np.histogram(markers_basis.flatten(), bins=100)[
    0
]  # reason for choosing 0.3 below
...
labels_roi = skimage.util.img_as_ubyte(
    skimage.segmentation.watershed(gradient, markers)
)
labels_roi2 = skimage.util.img_as_ubyte(
    skimage.transform.resize(labels_roi, image_color[roi].shape[:2])
)
...
sub_image = skimage.img_as_ubyte(
    skimage.restoration.denoise_tv_bregman(sub_image, weight=1, channel_axis=-1)
)
...
# process the watershed and resize to the original size
sub_labels_roi = skimage.util.img_as_ubyte(
    skimage.segmentation.watershed(gradient, markers)
) + int(np.max(labels_roi))
sub_labels_roi2 = skimage.util.img_as_ubyte(
    skimage.transform.resize(sub_labels_roi, image_color[roi].shape[:2])
)
